...
def build_scalar_dataset(
    ...
    """Sample ``n_samples`` random triangles and evaluate scalar quantities.

    Returns ``(names, data)`` where ``data`` has shape ``(n_samples, len(names))``.
    All requested triangles are drawn once and every requested scalar is
    evaluated on the same set of triangles, so different 3-scalar combinations
